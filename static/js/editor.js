...
x$.controller('plEditor', ['$scope', '$http', '$timeout', '$interval', '$sce', 'plConfig', 'IOService', 'dataService', 'chartService', 'paletteService', 'themeService', 'plNotify', 'eventBus', 'permService', 'license'].concat(function($scope, $http, $timeout, $interval, $sce, plConfig, IOService, dataService, chartService, paletteService, themeService, plNotify, eventBus, permService, license){
  ...
  */
  import$($scope, {
    ...
    _save: function(nothumb){
      var key, refresh, k, data, this$ = this;
    ...
    download: {
      track: function(item){
        return ga('send', 'event', 'ChartEditor', 'Download', item.name, $scope.target().key);
      },
      prepare: function(){
        var this$ = this;
  ...
  import$($scope, {
        ...
        return ['#edit-sharelink', '#edit-embedcode'].map(function(eventsrc){
          ...
          embedcodeGenerator = function(){
            ...
            if ($scope.sharePanel.aspectRatio) {
              ...
              return "<iframe src=\"" + link + "\" width=\"" + w + "\" height=\"" + h + "\" allowfullscreen=\"true\" frameborder=\"0\"></iframe>";
            }
          };
    ...
    monitor: function(){
      ...
      this.$watch('chart.license.options', function(it){
        var ref$;
        if (!it) {
          ((ref$ = $scope.chart).license || (ref$.license = {})).options = license.init();
        }
        return ((ref$ = $scope.chart).license || (ref$.license = {})).name = license.match(((ref$ = $scope.chart).license || (ref$.license = {})).options);
      }, true);
      this.$watch('chart.license.name', function(){
        var ref$;
        if (((ref$ = $scope.chart).license || (ref$.license = {})).name !== 'Custom License') {
          ((ref$ = $scope.chart).license || (ref$.license = {})).options = license.get($scope.chart.license.name);
          return console.log($scope.chart.license.options);
        }
      }, true);
