...
var plotdbDomain, sendMsg, loadscript, loadlib, store, errorHandling, properEval, saveLocal, configPreset, configParser, dispatcher, snapshot;
plotdbDomain = /plotdb/.exec(window.location.href) ? "https://plotdb.com/" : "http://localhost/";
...
dispatcher = function(evt){
  ...
  if (evt.data.type === 'set-config') {
    ...
    chart.config(configParser(evt.data.config));
    if (evt.data.rebind) {
      chart.parse();
    }
    chart.resize();
  ...
  if (evt.data.type === 'init') {
    ...
    loadlib(evt.data).then(function(){
      ...
      sendMsg({
        ...
        config: JSON.stringify(configPreset(store.module.config))
      });
