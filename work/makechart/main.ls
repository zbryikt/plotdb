...
io.query "select code,style,doc from charts where key = 2203" .then (r={}) ->
  code = r.rows.0.code
  ...
  doit = (county, count) ->
    ...
    ret = io.query("update charts set assets = $1 where key = #count", [JSON.stringify(object.assets)])
      .then (r={}) ->
        ccode = JSON.parse(JSON.stringify(code))
        cstyle = JSON.parse(JSON.stringify(style))
        cdoc = JSON.parse(JSON.stringify(doc))
        ccode.content = ccode.content.replace(/屏東縣/, "#county")
        desc = """動畫式的#{county}村里級熱圖，透過點擊縣市中的鄉鎮再顯示村里細節。資料以村里為主，但可選擇鄉鎮區塊的資料計算方式要透過村里數值的加總或平均，亦可自由控制是否要顯示鄉鎮區塊。"""
        io.query(
          "update charts set (code,style,doc,name,owner,description) = ($1,$2,$3,$4,$5,$6) where key = #count",
          [JSON.stringify(ccode),JSON.stringify(cstyle),JSON.stringify(cdoc),"#{county}互動地圖", 4,desc]
        )
    return ret
