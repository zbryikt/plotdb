...
files = fs.readdir-sync \cwb/ .map ->
  {name: it.replace(/\.csv$/,""), path: "cwb/#it"}
...
user = do
  username: "cwb.data@plotdb.com"
  password: ""
  usepasswd: false
  displayname: "Central Weather Bureau"
  createdtime: new Date!
  avatar: 0

keys = []
owner = 0
console.log "find cwb owner..."
io.query "select * from users where username=$1", [user.username]
  ...
  .then (r={}) ->
    owner := r.[]rows.0.key
    console.log "cwb owner key = #owner, listing all datasets from it..."
  ...
  .then ->
    console.log "drop all dataset from cwb..."
