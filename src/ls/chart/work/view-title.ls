...
plotdb.view.chart.prototype <<< do
  ...
  attach: (root) ->
    ...
    iroot = document.createElement("div")
    iiroot = document.createElement("div")
    iroot.appendChild(iiroot)
    iroot.setAttribute("id", "iroot")
    iiroot.setAttribute("id", "iiroot")
    iroot.style <<< {position:"absolute",left:"0",right:"0"}
    iiroot.style <<< {width: "100%",height: "100%"}
    root.appendChild(head)
    root.appendChild(iroot)
    root.appendChild(foot)
    head.innerHTML = "<h2 style='text-align:center;margin:0'>Hello World!</h2>"
    foot.innerHTML = "<small>data source: <a href='#'>here</a>"
    foot.style <<< {position:"absolute",bottom: "0"}
    headbox = head.getBoundingClientRect!
    footbox = foot.getBoundingClientRect!
    iroot.style.top = headbox.height + "px"
    iroot.style.bottom = footbox.height + "px"
    ...
    chart.root = iiroot.querySelector("div:first-of-type")
    resize = ->
